...
export const LogForm = ({oname, editData, setEditRegister, setOpenEdit}) => {
    const toast = useToast()
    const userData = decodeSessionStorage().payload;
    const tableName = 'log'
    const [shift, setShift] = useState(editData ? editData.shift : null)
    const [commencingNo, setCommencingNo] = useState(editData ? editData.commencing_number : null)
    const [closingNo, setClosingNo] = useState(editData ? editData.closing_number : null)
    const [itc, setItc] = useState(editData ? editData.ITC : null)
    const [ni, setNi]=useState(editData ? editData.NI : null)
    const [can,setCan]=useState(editData ? editData.CAN : null)
    const [sp,setSp]=useState(editData ? editData.SPCAN : null)
    const [noofpass,setNoofpass]=useState(editData ? editData.Number_of_pass : null)
    const [cash,setCash]=useState(editData ? editData.CASH : null)
    const [vouncher,setVouncher]=useState(editData ? editData.Vouncher : null)
    const [pos,setPos]=useState(editData ? editData.POS : null)
    const [ecash,setEcash]=useState(editData ? editData.Ecash : null)
    const [ubi,setUbi]=useState(editData ? editData.UPI_PAYMENT : null)
    const [toc,setToc]=useState(editData ? editData.TOC : null)
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(`${process.env.REACT_APP_CONFIG}/addRegisters`, {
            tableName, date, shift, userid, loginTime, commencingNo, closingNo, logoutTime, itc, ni, can, sp, noofpass, cash, vouncher, pos, ecash, ubi, toc,oname, scode, registerid
        })
            .then(res => {
                toast({
    ...
    const handleUpdate = (e) => {
        ...
        axios.post(`${process.env.REACT_APP_CONFIG}/editLogRegisters`, {
            tableName, id: editData.id, shift, commencingNo, closingNo, itc, ni, can, sp, noofpass, cash, vouncher, pos, ecash, ubi, toc, oname, scode
        })
            .then(res => {
                toast({
                    description: "Successfully Added",
    ...
    return(
        ...
        />
            <DialogActions>
                <Button style={{backgroundColor: '#202950', color: 'white'}} variant="contained" onClick={() => {
                    if(setOpenEdit) setOpenEdit(false)
                    if(setEditRegister) setEditRegister({})
                }}>
                    Cancel
                </Button>
...
export const GbrForm=({oname}) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        axios.post(`${process.env.REACT_APP_CONFIG}/addGbrRegisters`, {
            tableName,oname,slno,frm,too,dates,address,trainno,doj,noofpass,classs,purpose,authority,scode, registerid
        })
            .then(res => {
                toast({
                    description: "Successfully Added",
    ...
    return(
        <form onSubmit={handleSubmit}>
            ...
            <TextField
                ...
                autoComplete="off"
                type="text"
                label="Address of passenger Mob. No"
                name="Address of passenger Mob. No"
                value={address}
                style={{marginRight: '40px',marginBottom: '20px'}}
            ...
            />
            <DialogActions>
                <Button type="" style={{backgroundColor: '#202950', color: 'white'}} variant="contained">
                    Cancel
                </Button>
                <Button type="submit" style={{backgroundColor: '#202950', color: 'white', marginRight:'20.5px'}} variant="contained">
                    Add 
